...
def recommend(path,epoch=100, top=20, device='cuda', base=20, recommend=80, ee=False,alpha=0.5):
    args = get_path(path)
    intention_size = 100
    user_size = 50
        ...
        for k in range(epoch):
            ...
            for i in range(top):
                for intention in intention_list:
                    ...
                    if ee:
                        predict.append(record(intention,model.get_infomax(user, intention) + correct[intention] * alpha / times[intention] + math.sqrt(2 * math.log(i, 2) / times[intention])))
                    else:
                        predict.append(record(intention, model.get_infomax(user, intention)))
                predict = sorted(predict)
                item = random.sample(predict[0].intention.node_list,1)[0]
                if item.id in user.item_list:
                    correct[predict[0].intention] +=1
                    true_cnt += 1
                    result.append(1)
                else:
                    result.append(0)
                times[predict[0].intention] +=1
