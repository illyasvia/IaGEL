...
class intention:
    ...
    def destroy(self):
        op = random.random()
        if op > 0.5:
            temp = random.randint(1, len(self.node_list))
            self.node_list[temp].feature = torch.randn_like(self.node_list[temp].feature)
        else:
            temp = random.randint(1, len(self.edge_index[0]))
            self.edge_index[0].pop(temp)
            self.edge_index[1].pop(temp)
